    ...
    function computeNodeLinks() {
        ...
        links.forEach(function(link) {
            var source = link.source,
                target = link.target;
            if (typeof source === "number") source = link.source = returnNode(link.source);
            if (typeof target === "number") target = link.target = returnNode(link.target);
            source.sourceLinks.push(link);
            target.targetLinks.push(link);
        });
    }
    //dataId return node name
    function returnNode(dataId) {
        let no = null;
        nodes.forEach(function(node) {
            if(node.id==dataId){
                no =  node;
        ...
        if (no) return no;
        else return {name:'未定义',sourceLinks:[],targetLinks:[]};
    }
